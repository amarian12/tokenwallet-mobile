...
export class DashboardPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.appflow.walletBalances.subscribe(walletBalances => {
      ...
      if(coinInfo){
        this.fiatValue  = this.currencyPipe.transform(coinInfo.price_fiat, this.marketService.coinMarketInfo.selected_fiat, 'symbol', '1.2-8');
        this.coinSupply = coinInfo.total_supply;
        this.marketCapital = this.currencyPipe.transform(coinInfo.market_cap_fiat, this.marketService.coinMarketInfo.selected_fiat, 'symbol', '1.2-2');
        this.marketVolumeUSD =  this.currencyPipe.transform(coinInfo.market_24h_volume_fiat, this.marketService.coinMarketInfo.selected_fiat, 'symbol', '1.2-2');
      }
      if(walletBalances){
